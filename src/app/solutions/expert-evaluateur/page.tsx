...
export async function generateMetadata(): Promise<Metadata> {
  const [t, locale] = await Promise.all([
    getTranslations("solutions.expert-evaluateur.meta"),
    getLocale(),
  ]);
  return {
    title: t("title"),
    description: t("description"),
    alternates: localizedAlternates("/solutions/expert-evaluateur", locale),
  };
}

export default async function ExpertEvaluateurSolutionPage() {
  const [t, locale] = await Promise.all([
    getTranslations("solutions.expert-evaluateur"),
    getLocale(),
  ]);
  ...
  return (
    <PersonaLanding
      ...
      finalCta={{
        ...
        ctaSecondaryHref: `${lp}/valorisation`,
      }}
    />
  );
}
