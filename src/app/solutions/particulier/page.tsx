...
export async function generateMetadata(): Promise<Metadata> {
  const [t, locale] = await Promise.all([
    getTranslations("solutions.particulier.meta"),
    getLocale(),
  ]);
  return {
    title: t("title"),
    description: t("description"),
    alternates: localizedAlternates("/solutions/particulier", locale),
  };
}

export default async function ParticulierSolutionPage() {
  const [t, locale, ttoc] = await Promise.all([
    getTranslations("solutions.particulier"),
    getLocale(),
  ...
  return (
    <PersonaLanding
      ...
      finalCta={{
        ...
        ctaSecondaryHref: `${lp}/estimation`,
      }}
      toc={{
        label: ttoc("label"),
        hero: ttoc("hero"),
