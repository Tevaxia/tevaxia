...
function fmtDateTime(s: string | null): string {
  if (!s) return "jamais";
  const d = new Date(s);
  return d.toLocaleString("fr-LU", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" });
}
...
export default function ChannelsPage(props: { params: Promise<{ propertyId: string }> }) {
  ...
  const submitNew = async () => {
    const urlErr = iCalUrlValidation(form.ics_url);
  ...
  const handleSync = async (cal: PmsExternalCalendar) => {
    ...
    try {
      ...
      if (res.ok) {
        setFlash(`${SOURCE_LABELS[cal.source]} : +${res.imported} importés, ${res.updated} mis à jour, ${res.deactivated} annulés.`);
      } else {
        setError(res.error || "Échec sync");
      }
      await reload();
    } catch (e) {
      setError(errMsg(e));
  ...
  const handleSyncAll = async () => {
    ...
    await reload();
    setSyncingAll(false);
    setFlash("Synchronisation globale terminée.");
  };
  ...
  const handleDelete = async (cal: PmsExternalCalendar) => {
    if (!confirm(`Supprimer le flux "${cal.label}" ? Les réservations blocks importées ne seront pas supprimées automatiquement.`)) return;
    await deleteCalendar(cal.id);
    await reload();
  };

  // Autofill label + source à partir de l'URL
  const handleUrlChange = (v: string) => {
    const next = { ...form, ics_url: v };
    if (v && form.source === "airbnb" && !form.label) {
      next.source = detectSourceFromUrl(v);
      next.label = SOURCE_LABELS[next.source];
    } else if (v) {
      next.source = detectSourceFromUrl(v);
    }
    setForm(next);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-navy">Channels — import iCal</h1>
          <p className="mt-1 text-sm text-muted">
            Synchronisation automatique des calendriers OTA (Airbnb, Booking, VRBO, Expedia…) vers
            les réservations PMS. One-way, évite les double-bookings. Pour la distribution à deux sens,
            un channel manager (SiteMinder, Cloudbeds) sera branché en T4.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowForm(!showForm)}
            ...
            {showForm ? "Annuler" : "+ Ajouter un flux iCal"}
          </button>
        </div>
      </div>
      ...
      {calendars.length > 0 && (
        <div className="mt-5 grid gap-3 sm:grid-cols-4">
          <StatCard label="Flux actifs" value={calendars.filter((c) => c.active).length} />
          <StatCard label="Total flux" value={calendars.length} />
          <StatCard label="Blocs actifs (aujourd'hui)" value={totalActive} tone="emerald" />
          <StatCard label="Blocs à venir" value={totalFuture} tone="blue" />
        </div>
      )}
      ...
      {showForm && (
        <div className="mt-5 rounded-xl border border-navy/20 bg-navy/5 p-5 space-y-3">
          <h2 className="text-sm font-bold uppercase tracking-wider text-navy">Nouveau flux iCal</h2>
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-xs sm:col-span-2">
              <div className="mb-1 font-semibold text-slate">URL iCal *</div>
              ...
              <div className="mt-1 text-[10px] text-muted">
                Sur Airbnb : Calendrier → Sync avec d&apos;autres calendriers → Exporter. Sur Booking : Extranet → Rates & Availability → Sync calendars.
              </div>
            </label>
            <label className="text-xs">
              <div className="mb-1 font-semibold text-slate">Source</div>
            ...
            <label className="text-xs">
              <div className="mb-1 font-semibold text-slate">Libellé *</div>
              <input type="text" value={form.label}
                onChange={(e) => setForm({ ...form, label: e.target.value })}
                placeholder={`${SOURCE_LABELS[form.source]} — Chambre Deluxe`}
                className="w-full rounded-lg border border-input-border bg-input-bg px-3 py-2 text-sm" />
            </label>
            <label className="text-xs">
              <div className="mb-1 font-semibold text-slate">Chambre ciblée (optionnel)</div>
            ...
            <label className="text-xs">
              <div className="mb-1 font-semibold text-slate">Type de chambre (optionnel)</div>
      ...
      {calendars.length === 0 ? (
        <div className="mt-6 rounded-xl border-2 border-dashed border-card-border py-12 text-center text-sm text-muted">
          Aucun flux iCal configuré. Ajoutez votre premier flux Airbnb / Booking pour synchroniser automatiquement
          les réservations entrantes et éviter les double-bookings.
        </div>
      ) : (
            ...
            return (
              <div key={c.id} className={`rounded-xl border p-4 ${isError ? "border-rose-200 bg-rose-50/40" : "border-card-border bg-card"}`}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {!c.active && (
                        <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-700">Désactivé</span>
                      )}
                    ...
                    {(c.room_id || c.room_type_id) && (
                      <div className="mt-1 text-[11px] text-muted">
                        Scope : {c.room_id
                          ? `Chambre ${rooms.find((r) => r.id === c.room_id)?.number ?? "?"}`
                          : `Type ${roomTypes.find((rt) => rt.id === c.room_type_id)?.name ?? "?"}`}
                      </div>
                    )}
                  </div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5 text-[11px]">
                  <StatMini label="Dernière sync" value={fmtDateTime(c.last_sync_at)} />
                  <StatMini label="Sync totaux" value={String(c.sync_count)} />
                  <StatMini label="Events import." value={String(c.events_count)} />
                  <StatMini label="Blocs actifs" value={String(st?.active_blocks ?? 0)} tone="emerald" />
                  <StatMini label="Blocs futurs" value={String(st?.future_blocks ?? 0)} tone="blue" />
                </div>

                {isError && c.last_error && (
                  <div className="mt-2 rounded border border-rose-200 bg-rose-50 px-2 py-1 text-[11px] text-rose-900">
                    <strong>Erreur :</strong> {c.last_error}
                  </div>
                )}
              </div>
            );
      ...
      <div className="mt-6 rounded-xl border border-blue-200 bg-blue-50 p-4 text-xs text-blue-900">
        <strong>Bon à savoir :</strong> les OTA rafraîchissent leurs flux iCal avec un délai de 15 min à 3h
        (variable selon le portail). Pour éviter les doubles-réservations, synchronisez plusieurs fois par jour
        ou activez le cron automatique (à venir). L&apos;export iCal de tevaxia (flux sortant pour nourrir les OTA)
        est disponible sur la page réservations.
      </div>
    </div>
  );
}
