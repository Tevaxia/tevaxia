...
export default function RapprochementPage() {
  const t = useTranslations("syndicRapprochement");
  ...
  const authFetch = useCallback(async (input: string, init?: RequestInit) => {
    ...
    if (!token) throw new Error(t("errLoginRequired"));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error(data.error ?? t("errCodeExchange"));
  ...
  }, [user, authFetch, coownershipId, psd2Country, t]);
  ...
  const fetchPsd2Transactions = async (accountUid: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? t("errFetchTx"));
      ...
      const credits = txs.filter((tx) => tx.amount > 0).reduce((s, tx) => s + tx.amount, 0);
      const debits = txs.filter((tx) => tx.amount < 0).reduce((s, tx) => s + tx.amount, 0);
      setStatement({
        transactions: txs,
  ...
  const applyMatch = async (m: MatchResult) => {
    ...
    setFlash(t("flashPaid", { lot: m.matched_charge.lot_number }));
  ...
  const applyAll = async () => {
    ...
    if (!confirm(t("confirmBulk", { n: toApply.length }))) return;
    setApplying(true);
    let done = 0;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <div className="mt-5 rounded-xl border border-card-border bg-card p-4">
        <div className="text-xs font-semibold uppercase tracking-wider text-muted">{t("stateTitle")}</div>
        <div className="mt-1 flex flex-wrap gap-4 text-sm">
          <div>{t("stateCharges", { n: unpaid.length })}</div>
          <div>{t("stateOutstanding", { v: formatEUR(unpaid.reduce((s, u) => s + u.outstanding, 0)) })}</div>
        </div>
      </div>
      ...
      {!statement && (
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          ...
          <div className="rounded-xl border-2 border-dashed border-navy/20 bg-navy/5 p-8 text-center">
            ...
            <h2 className="text-sm font-bold text-navy">{t("csvTitle")}</h2>
            <p className="mt-1 text-[11px] text-muted">{t("csvHint")}</p>
          ...
          <div className="rounded-xl border-2 border-dashed border-emerald-300 bg-emerald-50/30 p-8 text-center">
            <div className="text-3xl mb-2">🏦</div>
            <h2 className="text-sm font-bold text-emerald-900">{t("psdTitle")}</h2>
            ...
            {psd2Step === "select-bank" && (
              <div className="mt-4 text-left">
                {psd2Enabled === false ? (
                  ...
                  <div className="text-center text-xs text-muted">{t("psdLoading")}</div>
                ) : (
                  <>
                    <div className="mb-2 flex items-center justify-between">
                      <label className="text-[11px] font-semibold text-slate">
                        ...
                        <select value={psd2Country} onChange={(e) => { setPsd2Country(e.target.value); setPsd2Step("idle"); }}
                          ...
                          <option value="LU">{t("cLu")}</option>
                          <option value="BE">{t("cBe")}</option>
                          <option value="FR">{t("cFr")}</option>
                          <option value="DE">{t("cDe")}</option>
                          <option value="NL">{t("cNl")}</option>
                        </select>
                      </label>
                      <button onClick={() => setPsd2Step("idle")} className="text-[11px] text-muted hover:text-navy">{t("btnCancel")}</button>
                    </div>
            ...
            {psd2Step === "accounts" && psd2Accounts.length > 0 && (
              <div className="mt-4 text-left">
                ...
                <div className="text-[11px] font-semibold text-slate mb-1">{t("psdAccounts")}</div>
                <div className="space-y-1">
                  {psd2Accounts.map((acc) => (
                    <button key={acc.uid} onClick={() => fetchPsd2Transactions(acc.uid)}
                      ...
                      <div>
                        <div className="text-xs font-semibold text-navy">{acc.name ?? t("defaultAccount")}</div>
                        <div className="text-[10px] text-muted font-mono">{acc.account_id?.iban ?? acc.uid}</div>
                      </div>
                      <span className="text-[10px] text-emerald-700 font-semibold">{t("btnImport")}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {psd2Step === "fetching" && (
              <div className="mt-4 text-xs text-emerald-900">{t("psdFetching")}</div>
            )}
          </div>
        </div>
      )}
      ...
      {statement && matches.length > 0 && (
        <>
          <div className="mt-5 grid gap-3 sm:grid-cols-4">
            <Stat label={t("kpiTx")} value={String(statement.total)} />
            <Stat label={t("kpiReliable")} value={String(matchedCount)} tone="emerald" />
            <Stat label={t("kpiUncertain")} value={String(uncertainCount)} tone="amber" />
            <Stat label={t("kpiUnmatched")} value={String(unmatchedCount)} tone="rose" />
          </div>
          ...
          <div className="mt-5 flex justify-between">
            ...
            {matchedCount > 0 && (
              <button onClick={applyAll} disabled={applying}
                ...
                {applying ? t("btnApplying") : t("btnApplyReliable", { n: matchedCount })}
              </button>
            )}
          </div>
              ...
              return (
                <div key={i} className={`rounded-xl border p-4 ${
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {/* Transaction */}
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-muted font-semibold">{t("txLabel")}</div>
                      <div className="mt-1 text-lg font-bold text-navy">
                        {m.tx.amount > 0 ? "+" : ""}{formatEUR(m.tx.amount)}
                      </div>
                      <div className="text-xs text-muted">{new Date(m.tx.date).toLocaleDateString(dateLocale)}</div>
                      <div className="mt-1 text-xs">{m.tx.label}</div>
                      {m.tx.reference && (
                        <div className="text-[11px] font-mono text-muted">{t("refLabel", { r: m.tx.reference })}</div>
                      )}
                      {m.tx.counterparty && (
                    ...
                    <div>
                      {m.matched_charge ? (
                        <>
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${
                              ...
                              {t("scoreLabel", { s: m.match_score })}
                            </span>
                          </div>
                          <div className="mt-1 text-sm font-semibold text-navy">
                            {t("matchOwner", { lot: m.matched_charge.lot_number, name: m.matched_charge.owner_name ?? t("unknownOwner") })}
                          </div>
                          <div className="text-xs text-muted">{m.matched_charge.call_label}</div>
                          <div className="text-xs font-mono">
                            {t("matchDue", { due: formatEUR(m.matched_charge.outstanding), ref: m.matched_charge.payment_reference ?? t("matchUnknownRef") })}
                          </div>
                          <div className="text-[10px] text-muted italic mt-1">
                            {t("matchCriteria", { r: m.match_reason })}
                          </div>
                        ...
                        <div className="text-sm text-rose-700 font-semibold">{t("noMatch")}</div>
                      )}
                    </div>
                  </div>
                </div>
      ...
      <div className="mt-6 rounded-xl border border-blue-200 bg-blue-50 p-4 text-xs text-blue-900">
        <strong>{t("adviceTitle")}</strong> {t("adviceBody")}
      </div>
    </div>
  );
}
