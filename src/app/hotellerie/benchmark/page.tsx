...
function computeScore(h: HotelWithLatestPeriod): number {
  ...
  const occ = (p.occupancy ?? 0) * 100; // 0-100
  const adrNorm = Math.min(100, (p.adr ?? 0) / 3); // 300€ ADR = 100
  const gopMargin = (p.gop_margin ?? 0) * 100; // 0-100
...
export default function HotelBenchmarkPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // Fetch latest period for each hotel
      if (supabase) {
    ...
    return (
      <div className="mx-auto max-w-4xl px-4 py-12">
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-6 text-amber-900">
          Supabase requis + org_type=hotel_group (migration 013).
        </div>
      </div>
    );
  }
  ...
  if (orgs.length === 0) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-12">
        ...
        <h1 className="mt-2 text-2xl font-bold text-navy">Benchmark inter-hôtels</h1>
        <div className="mt-6 rounded-xl border-2 border-dashed border-card-border py-12 text-center text-sm text-muted">
          Vous n&apos;avez pas encore d&apos;organisation de type &quot;groupe hôtelier&quot;.
          Créez-en une depuis <Link href="/profil/organisation" className="text-navy underline">/profil/organisation</Link>.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      ...
      <p className="mt-1 text-sm text-muted">
        Classement des hôtels de votre groupe par score composite (occupation 35 % · ADR 30 % · GOP margin 35 %).
        Identifie les outliers à optimiser.
      </p>
      ...
      {stats && (
        <div className="mt-6 grid gap-3 sm:grid-cols-4">
          <div className="rounded-xl border border-card-border bg-card p-4 text-center">
            <div className="text-xs text-muted">Hôtels actifs</div>
            <div className="mt-1 text-2xl font-bold text-navy">{stats.count}</div>
          </div>
          <div className="rounded-xl border border-card-border bg-card p-4 text-center">
            <div className="text-xs text-muted">RevPAR moyen</div>
      ...
      {ranked.length === 0 ? (
        <div className="mt-8 rounded-xl border-2 border-dashed border-card-border py-12 text-center text-sm text-muted">
          Aucun hôtel enregistré dans cette organisation.
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-card-border bg-background/60">
                <th className="px-3 py-3 text-left font-semibold text-navy">#</th>
                <th className="px-3 py-3 text-left font-semibold text-navy">Hôtel</th>
                <th className="px-3 py-3 text-left font-semibold text-navy">Cat.</th>
                <th className="px-3 py-3 text-right font-semibold text-navy">Chambres</th>
                <th className="px-3 py-3 text-right font-semibold text-navy">RevPAR</th>
                <th className="px-3 py-3 text-right font-semibold text-navy">ADR</th>
                <th className="px-3 py-3 text-right font-semibold text-navy">Occ.</th>
                <th className="px-3 py-3 text-right font-semibold text-navy">GOP %</th>
                ...
                return (
                  <tr key={h.id} className="border-b border-card-border/40 hover:bg-background/40">
                    ...
                    <td className="px-3 py-3">
                      ...
                      {p && <div className="text-[10px] text-muted">Dernière période : {p.period_label}</div>}
                    </td>
                    ...
                    <td className="px-3 py-3 text-right font-mono font-semibold">
                      ...
                      {p?.revpar && stats && (
                        <div className={`text-[9px] ${vsAvg > 0 ? "text-emerald-700" : "text-rose-700"}`}>
                          {vsAvg > 0 ? "+" : ""}{(vsAvg * 100).toFixed(1)} % vs moy.
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-3 py-3 text-right">
                      <span className={`inline-block rounded-lg border px-2 py-1 font-mono font-bold ${pctColor(h.score)}`}>
                        {h.score}
                      </span>
                    </td>
                  </tr>
      ...
      <div className="mt-6 rounded-xl border border-blue-200 bg-blue-50 p-4 text-xs text-blue-900">
        <strong>Scoring :</strong> occupation (35 %) + ADR normalisé 300 € = 100 (30 %) + GOP margin (35 %).
        ≥ 75 excellent · 60-75 bon · 45-60 à surveiller · &lt; 45 préoccupant. Permet d&apos;identifier
        les outliers à cloner (top) ou optimiser (bottom). Cf. <Link href="/hotellerie/observatoire-lu" className="underline">observatoire LU</Link> pour le
