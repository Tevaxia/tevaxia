    ...
    return (
      <div className="bg-background py-16">
        <div className="mx-auto max-w-md px-4 text-center">
          <p className="text-muted">Authentication not configured.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-background py-16">
        <div className="mx-auto max-w-md px-4">
          <div className="rounded-xl border border-card-border bg-card p-8 shadow-sm text-center">
            ...
            <h2 className="text-lg font-semibold text-navy">Signed in</h2>
            <p className="mt-1 text-sm text-muted">{user.email}</p>
            <div className="mt-6 space-y-3">
              <Link href="/lb/mes-evaluations" className="block rounded-lg bg-navy px-4 py-2.5 text-sm font-medium text-white hover:bg-navy-light transition-colors">
                My valuations
              </Link>
        ...
        <div className="rounded-xl border border-card-border bg-card p-8 shadow-sm">
          <div className="text-center mb-6">
            ...
            <p className="mt-1 text-sm text-muted">
              {mode === "login"
                ? "Access your saved valuations"
                : "Save your valuations in the cloud"}
            </p>
          </div>
            ...
            <button
              onClick={async () => {
                ...
                await supabase.auth.signInWithOAuth({
                  provider: "google",
                  options: { redirectTo: `${window.location.origin}/en/connexion` },
                });
              }}
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center"><span className="bg-card px-3 text-xs text-muted">or by email</span></div>
          </div>
          ...
          <p className="mt-4 text-center text-xs text-muted">
            ...
            <Link href="/lb/mentions-legales" className="text-navy hover:underline">legal notice</Link>
            {" "}and our{" "}
            <Link href="/lb/confidentialite" className="text-navy hover:underline">privacy policy</Link>.
          </p>
        </div>
      </div>
    </div>
