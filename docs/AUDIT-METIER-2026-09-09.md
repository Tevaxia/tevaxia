...
## 12 septembre — migration 066 appliquée sur Supabase

- Autorisation utilisateur de mettre en œuvre le correctif serveur. Projet vérifié : dpynqvilgniohgtichbz, PostgreSQL 17.6. Application administrative via l'éditeur SQL, contrôle final horodaté 2026-09-12T21:41:38Z (23:41 Paris).
- Définitions et droits réels sauvegardés avant modification. Schéma des trois tables compatible avec 005/033/034, jeton UNIQUE et aucun déclencheur applicatif. Les anciennes fonctions réellement installées présentaient les courses décrites.
- 066 verrouille le lien avant admission, vérifie l'heure après attente, conserve compteur et journal dans la même transaction, borne les commentaires. Search path fixé et attente de verrou limitée à cinq secondes. Aucun enregistrement existant supprimé ou réécrit par la migration.
- Test indépendant avec huit connexions PostgreSQL 17.6 locales : anciennes fonctions = huit vues malgré plafond 1 et huit commentaires simultanés ; fonctions corrigées = une admission et sept refus dans les deux cas. Expiration après attente de verrou vérifiée pour lecture et commentaire. Serveur local arrêté. Tests PGlite complémentaires réussis (échec du journal avec annulation du compteur, saturation, champs, rôles, réapplication).
- Application SQL réussie, corps des deux fonctions comparés au SQL testé (hors indentation), paramètres et SECURITY DEFINER confirmés. Droits EXECUTE anon/authenticated/service_role conservés ; droit implicite PUBLIC retiré. Les deux RPC HTTP publiques renvoient 200/not_found avec un jeton malformé ; aucun vrai lien client consulté et aucun commentaire de production créé pour les essais.
- Preuves et retour arrière : C:/Users/erwan/Documents/Codex/2026-09-12/tes/outputs/066_production_before.json, 066_rollback.sql, 066_production_verification.json, 066_concurrency_validation.log et 066_api_validation.log.
- Contrôle en lecture seule à 21:42:53Z : les trois politiques restrictives attendues de 064/065 sont absentes de la production ; RLS est activé sur les trois tables. Ces anciens lots restent à comparer/tester/appliquer séparément. Le présent lot n'est pas une certification de toutes les politiques RLS/MFA ni une idempotence persistante des commentaires.

## 12–13 septembre — protections locatives appliquées et tableau de bord fiabilisé

- Demande utilisateur explicite : appliquer 064/065, fiabiliser les agrégats, poursuivre les contrôles des droits/parcours et pousser sur GitHub.
- Supabase Tevaxia dpynqvilgniohgtichbz, PostgreSQL 17.6 : schéma et politiques actuels examinés sans lire de lignes clients ; définition/ACL du portail sauvegardées avant application. Les politiques antérieures vérifiaient le propriétaire de chaque ligne, sans garantir que celui-ci possède le lot référencé.
- 064/065 appliquées dans une transaction administrative, résultat Success. Les trois politiques RESTRICTIVE FOR ALL vérifient propriétaire de la ligne ET du lot, avec USING et WITH CHECK. Politiques existantes conservées. Fonction du portail liée au propriétaire du lot et des paiements ; verrous sur jeton et lot ; expiration vérifiée à l'heure courante après attente ; compteur saturé sans dépassement ; historique limité à 24 périodes. Search path vide et lock_timeout cinq secondes. EXECUTE anon/authenticated/service_role conservé et droit implicite PUBLIC retiré. Aucun enregistrement existant réécrit ou supprimé.
- Vérification production à 2026-09-12T21:55:07Z : trois politiques présentes, clauses cohérentes, fonction et paramètres installés. RPC anonyme HTTP 200/invalid_token avec jeton NULL, sans utiliser un lien réel. Les migrations SQL ne sont pas exécutées par Vercel.
- Tests SQL PGlite puis PostgreSQL natif 17.6 : deux propriétaires fictifs, reproduction de la fuite historique par jeton incohérent ; refus des lectures/inserts/réaffectations/suppressions entre propriétaires ; opérations légitimes préservées même en présence de politiques permissives larges ; colocataires, historique, révocation, expiration, réapplication. Tests avec connexions distinctes : expiration après attente du jeton et changement du propriétaire du lot pendant attente refusés. Serveur local arrêté après les tests.
- Tableau de bord personnel : nouvelles lectures bornées, jeton de session capturé et utilisateur vérifié ; filtres user_id explicites ; recontrôle du compte avant/après lecture ; seules les colonnes nécessaires sont chargées. Les mandats sont paginés par identifiant jusqu'à une page vide, même si le serveur renvoie une page courte ; ordre/doublons/données invalides et échec d'une page invalident l'agrégat. Limite 10 000 lignes/60 secondes avec erreur explicite. Activité : dix entrées récentes, ordre stable, nombre exact vérifié, aucune métadonnée privée inutile. Lots : HEAD/count exact, un échec n'est jamais un zéro.
- Agrégats : tous les cinq statuts de mandat en cours sont inclus (signé, diffusé, visite, offre, compromis). Commissions de ventes déclarées reçues additionnées en centimes ; zéro explicite conservé et une commission manquante rend la somme indisponible. Suppression de la somme trompeuse de tous les calculs présentée comme patrimoine : compteur de calculs immobiliers sauvegardés, avec mention que plusieurs calculs peuvent concerner le même bien. Les vues des liens sont bornées par la précision des entiers et les expirations réévaluées toutes les trente secondes.
- Interface : erreurs par source, tirets et message d'indisponibilité au lieu d'un état vide ; reprise explicite ; remise à zéro lors du changement de compte ; réponses tardives ignorées. Montants localisés avec centimes, textes dans les cinq langues, zoom 1,1 conservé.
- Contrôles des droits de production poursuivis sur les tables du tableau de bord : les mandats autorisent aussi la lecture par membres d'organisation ; le nouveau filtre propriétaire empêche d'intégrer ces mandats tiers aux totaux personnels. Le journal d'activité est lié au propriétaire. Les politiques locatives ont été comparées au schéma réel puis éprouvées sur des données fictives ; aucune opération financière réelle exécutée.
- Validation : 2 138 tests/178 fichiers dont 22 nouveaux ; TypeScript et build 1 347 pages réussis. Fixture page et service réels, React StrictMode, session/réseau simulés : erreur/reprise, indisponible distinct de zéro, centimes, commissions manquantes, changement de compte et réponse tardive, cinq langues, quatre largeurs 320/390/768/1440, zoom 1,1. Capture allemande 320 px inspectée.
- Preuves et retour arrière : C:/Users/erwan/Documents/Codex/2026-09-12/tes/outputs/064_portal_before.json, 064-065-rollback.sql, 064-065-native-validation.log, 064-api-validation.log, dashboard-fixture-validation.log. Les anciens points de cet audit indiquant 064/065/066 non appliquées sont remplacés par les constats d'application ci-dessus et du lot 066 précédent.
- Limite de portée : ce lot couvre les droits locatifs et les lectures du tableau de bord décrits ci-dessus. Il ne certifie pas l'ensemble des RPC financiers de syndic/agence, MFA/AAL2, intégrations ni modèles fiscaux ; la revue générale continue sur ces périmètres.
